...
# This script is only for Hue White Product User Defined Feature Testing
...
def IPJ_16697(light):
    ...
    effect = '{"startup": {"mode": "powerfail"}}'
    uds_set(effect)
    requests.put("http://" + bridge_ip + "/api/" + bridge_user + "/lights/" + str(light) + "/state",'{"bri":200}')
    time.sleep(10)
...
def IPJ_16698(light):
    ...
    print('\033[1;33m IPJ-16698: Not implemented yet \033[0m\n')
    time.sleep(3)


def IPJ_16699(light):
    print("Executing case ID IPJ-16699 *** Flicker Verification ***")
    effect = '{"startup": {"mode": "custom", "customsettings": {"bri":1}}}'
    uds_set(effect)
    time.sleep(3)
    ippower.smart_power(1, 2)
    time.sleep(30)
