...
# default_yc = 0.41
# time_window = 25
# time_between_case = 10
# toggle_2x_switch_on = 5
# toggle_2x_switch_off = 30
...
def IPJ_16687(light):
    ...
    uds_set(effect)
    res_text = requests.put("http://"+bridge_ip+"/api"+bridge_user+"/lights/"+str(light)+"/state",'{"bri":100, "xy":[0.6915, 0.3083]}').text
    res = str(res_text)
    if 'success' in res:
        time.sleep(7*2)
        print("Power Cycle Lamp and Wait 20 seconds")
...
def IPJ_16689(light):
    ...
    requests.put("http://" + bridge_ip + "/api" + bridge_user + "/lights/" + str(light) + "/state", '{"on":true, "bri":1}')
    time.sleep(5)
    ...
    for effect in effects:
        ...
        if effect_dict['startup']['mode'] == 'powerfail':
            ...
            res_text = requests.put("http://" + bridge_ip + "/api" + bridge_user + "/lights/" + str(light) + "/state", '{"bri":254}').text
        res = str(res_text)
        time.sleep(8)
        if 'success' in res:
...
def IPJ_16691(light):
    ...
    while status is True:
        print("Scan lamp with bridge in Zigbee network, wait 60 seconds")
...
def IPJ_16699(light):
    ...
    effects = ['{"startup": {"mode": "custom", "customsettings": {"bri":1,"xy":[0.69, 0.31]}}}',
               '{"startup": {"mode": "custom", "customsettings": {"bri":1,"xy":[0.17, 0.70]}}}',
...
def IPJ_16703(light):
    ...
    effect = '{"startup": {"mode": "powerfail"}}'
    uds_set(effect)
    requests.put("http://" + bridge_ip + "/api" + bridge_user + "/lights/" + str(light) + "/state",'{"bri":254,"xy":[0.46, 0.41]}')
    bri, xc, yc = uds_get()
    time.sleep(7*2)
    if bri == default_bri and xc == default_xc and yc == default_yc:
    ...
    time.sleep(1)
    print("IPJ-16703: Power cycle lamp and wait 20 seconds")
    ippower.power_reset()
    time.sleep(20)
    bri, xc, yc = uds_get()
    if bri == default_bri and xc == default_xc and yc == default_yc:
...
def IPJ_16704(light):
    ...
    effect = '{"startup": {"mode": "powerfail"}}'
    uds_set(effect)
    requests.put("http://" + bridge_ip + "/api" + bridge_user + "/lights/" + str(light) + "/state",'{"bri":1, "xy":[0.1700, 0.7000]}')
    bri, xc, yc = uds_get()
    time.sleep(7*2)
    if bri == 1 and xc == 0.17 and yc == 0.70:
        ...
        print("Start transition")
    time.sleep(7*2)
    print("IPJ-16704: Power cycle lamp and wait 20 seconds")
    ippower.power_reset()
    time.sleep(20)
    bri, xc, yc = uds_get()
