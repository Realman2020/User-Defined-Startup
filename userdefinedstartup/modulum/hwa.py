...
def IPJ_16687(light):
    ...
    uds_set(effect)
    res_text = requests.put("http://"+bridge_ip+"/api"+bridge_user+"/lights/"+str(light)+"/state",'{"bri":100, "ct":153}').text
    res = str(res_text)
    if 'success' in res:
        time.sleep(7*2)
        print("Power cycle lamp and wait 20 seconds")
...
def IPJ_16688(light):
    ...
    effect = '{"startup": {"mode": "custom", "customsettings": {"bri":200, "ct":153}}}'
    res_text = uds_set(effect)
    res = str(res_text)
    if 'success' in res:
        print("Power Cycle Lamp and Wait 20 seconds")
...
def IPJ_16697(light):
    ...
    effect = '{"startup": {"mode": "powerfail"}}'
    uds_set(effect)
    requests.put("http://" + bridge_ip + "/api" + bridge_user + "/lights/" + str(light) + "/state",'{"bri":100,"ct":153}')
    time.sleep(10)
...
def IPJ_16703(light):
    ...
    if bri == default_bri and ct == 153:
        requests.put("http://"+bridge_ip+"/api"+bridge_user+"/lights/"+str(light)+"/state", '{"bri":1,"ct":366,"transitiontime":600}')
...
def IPJ_16704(light):
    ...
    time.sleep(7*2)
    if bri == 1 and ct == 454:
        requests.put("http://"+bridge_ip+"/api"+bridge_user+"/lights/"+str(light)+"/state", '{"bri":254,"ct":366,"transitiontime":600}')
        print("Start transition")
    time.sleep(7*2)
    print("Power cycle lamp and wait 20 seconds")
    ippower.power_reset()
